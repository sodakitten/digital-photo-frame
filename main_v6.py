...
class CalendarBadge(QWidget):
    ...
    def paintEvent(self, event):
        ...
        for index, day_text in enumerate(weekdays):
            ...
            painter.drawText(QRect(x, 38, 28, 18), Qt.AlignmentFlag.AlignCenter, day_text)
        ...
        for row, week in enumerate(month_days):
            for col, day in enumerate(week):
                ...
                if day == today.day:
                    painter.setPen(Qt.PenStyle.NoPen)
                    painter.setBrush(QColor(0, 120, 215, 210))
                    painter.drawRoundedRect(rect.adjusted(2, 0, -2, 0), 6, 6)
...
class SettingsPage(QWidget):
    ...
    def updateLowPowerBtn(self):
        self.btn_low_power.setText("▼ 开启" if self.current_low_power_mode else "▼ 关闭")
        self._refreshSummary()

    def updateCalendarBtn(self):
        self.btn_calendar.setText("▼ 开启" if self.current_show_calendar else "▼ 关闭")
        self._refreshSummary()
...
class SlideshowPage(QWidget):
    ...
    def setupOverlay(self):
        ...
        self.overlay_buttons = [
            ...
            self.btn_fullscreen_overlay, self.btn_settings, self.btn_quit
        ]
...
class PhotoFrameApp(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("电子相框控制台 v5")
