...
SNOOZE_MINUTES = 10
SNOOZE_MINUTE_OPTIONS = (5, 10, 15, 30, 60)
...
def normalize_snooze_minutes(minutes):
    try:
        minutes = int(minutes)
    except Exception:
        minutes = SNOOZE_MINUTES
    return minutes if minutes in SNOOZE_MINUTE_OPTIONS else SNOOZE_MINUTES

def snooze_minutes_text(minutes):
    minutes = normalize_snooze_minutes(minutes)
    return "1小时" if minutes == 60 else f"{minutes}分钟"
...
class AlarmEditorDialog(QDialog):
    def __init__(self, parent, alarm=None):
        ...
        time_adjust_layout = QGridLayout()
        time_adjust_layout.setSpacing(10)
        time_adjust_buttons = [
            ("+1小时", 1, 0, 0, 0),
            ("+1分钟", 0, 1, 0, 1),
            ("-1小时", -1, 0, 1, 0),
        ...
        for text, hours, minutes, row, column in time_adjust_buttons:
            button = QPushButton(text)
            button.setMinimumHeight(54)
            button.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
            button.setStyleSheet("font-size: 20px; padding: 14px 18px;")
            button.clicked.connect(lambda _checked=False, h=hours, m=minutes: self.adjustTime(h, m))
            time_adjust_layout.addWidget(button, row, column)
        v.addLayout(time_adjust_layout)
    ...
    def adjustTime(self, hours=0, minutes=0):
        seconds = (hours * 60 + minutes) * 60
        self.time_edit.setTime(self.time_edit.time().addSecs(seconds))
...
class AlarmRingDialog(QDialog):
    def __init__(self, parent, alarm_time, ringtone_path="", alarm_id=""):
        super().__init__(parent)
        self.setWindowFlags(Qt.WindowType.FramelessWindowHint | Qt.WindowType.Dialog)
        self.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground)
        self.action = "dismiss"
        ...
        snooze_btn.setMinimumHeight(62)
        snooze_btn.setStyleSheet("font-size: 24px; font-weight: bold;")
        snooze_more_btn = QPushButton("时长选择")
        snooze_more_btn.setMinimumWidth(122)
        snooze_more_btn.setMinimumHeight(62)
        snooze_more_btn.setStyleSheet("font-size: 18px;")
        dismiss_btn = QPushButton("关闭闹钟")
        dismiss_btn.setMinimumHeight(62)
        dismiss_btn.setStyleSheet("background-color: #d9b66d; color: #1a1308; font-weight: bold;")
        snooze_btn.clicked.connect(self.snooze)
        snooze_more_btn.clicked.connect(self.chooseSnoozeMinutes)
        dismiss_btn.clicked.connect(self.dismiss)
        buttons.addWidget(snooze_btn, 2)
        buttons.addWidget(snooze_more_btn, 0)
        buttons.addWidget(dismiss_btn, 1)
        v.addLayout(buttons)
    ...
    def snooze(self):
        self._finish("snooze", SNOOZE_MINUTES)

    def chooseSnoozeMinutes(self):
        opts = {
            "5分钟后提醒": 5,
            "10分钟后提醒": 10,
            "15分钟后提醒": 15,
        ...
        dlg = TouchMenuDialog(self, "选择贪睡时长", opts)
        if dlg.exec():
            self._finish("snooze", dlg.selected_data)

    def dismiss(self):
        self._finish("dismiss")

    def _finish(self, action, snooze_minutes=None):
        self.action = action
        if action == "snooze":
            self.snooze_minutes = normalize_snooze_minutes(snooze_minutes)
        if self.media_player:
...
class SettingsPage(QWidget):
    ...
    def initUI(self):
        ...
        for label in (
            ...
            self.summary_random, self.summary_clock, self.summary_calendar,
            self.summary_alarm
        ):
    ...
    def onClockClicked(self):
        self.current_show_clock = not self.current_show_clock
        self.updateClockBtn()
        self.saveConfig()
...
class SlideshowPage(QWidget):
    ...
    def _positionCalendar(self):
        ...
        if hasattr(self, "overlay_widget") and not self.overlay_widget.isHidden():
            bottom_margin = max(bottom_margin, self.overlay_widget.height() + 36)
    ...
    def setupOverlay(self):
        ...
        self.overlay_layout.setContentsMargins(12, 8, 12, 8)
        self.overlay_layout.setHorizontalSpacing(6)
        self.overlay_layout.setVerticalSpacing(6)
    ...
    def _applyOverlayDensity(self, compact):
        if compact:
            font_size = 14
            padding = "8px 10px"
            min_width = 100
        ...
        """.format(font_size=font_size, padding=padding, min_width=min_width))

        self.overlay_layout.setContentsMargins(*margins)
        self.overlay_layout.setHorizontalSpacing(h_spacing)
        self.overlay_layout.setVerticalSpacing(v_spacing)
    ...
    def _arrangeOverlayButtons(self, force=False):
        ...
        self._applyOverlayDensity(compact)

        button_count = len(self.overlay_buttons)
        layout_changed = (
            force
            or getattr(self, "_overlay_columns", None) != columns
            or getattr(self, "_overlay_button_count", None) != button_count
            or getattr(self, "_overlay_compact", None) != compact
        )
        ...
        if columns == 2:
            overlay_width = max(300, min(560, width - 32))
        elif compact:
            overlay_width = min(680, max(560, int(width * 0.36)))
        else:
            overlay_width = min(920, max(720, int(width * 0.48)))
        self.overlay_widget.setFixedWidth(overlay_width)
...
class PhotoFrameApp(QWidget):
    def __init__(self):
        ...
        self._snooze_ringtone_path = ""
        self._snooze_minutes = SNOOZE_MINUTES
        self._snooze_alarm_id = ""
    ...
    def checkAlarms(self):
        ...
        if self._snooze_until:
            if not self._snoozeSourceAlarmActive():
                self._clearSnooze()
                self.refreshAlarmStatus()
                return
            ...
            if 0 <= delta <= ALARM_TRIGGER_WINDOW_SECONDS:
                ...
                ringtone_path = self._snoozeRingtonePath()
                alarm_id = self._snooze_alarm_id
                self._snooze_until = None
                self._snooze_ringtone_path = ""
                self._snooze_minutes = SNOOZE_MINUTES
                self._snooze_alarm_id = ""
                self._triggerAlarm(alarm_time, ringtone_path, alarm_id)
                return
        ...
        for alarm in self.settings_page.current_alarms:
            ...
            if 0 <= delta <= ALARM_TRIGGER_WINDOW_SECONDS and alarm.get("last_triggered_date") != date_key:
                ...
                self._triggerAlarm(alarm["time"], ringtone, alarm.get("id", ""))
                return

        if now.second == 0:
            self.refreshAlarmStatus()

    def _triggerAlarm(self, alarm_time, ringtone_path="", alarm_id=""):
        self._alarm_dialog_open = True
        self._alarm_was_playing = self.stacked.currentWidget() == self.slideshow_page and self.slideshow_page.play_timer.isActive()
        if self._alarm_was_playing:
        ...
        dlg = AlarmRingDialog(self, alarm_time, ringtone_path, alarm_id)
        self._alarm_dialog = dlg
        dlg.finished.connect(lambda _result=0, dialog=dlg: self._onAlarmDialogFinished(dialog))
        dlg.show()
        dlg.raise_()
        dlg.activateWindow()

    def _onAlarmDialogFinished(self, dialog):
        if self._alarm_dialog is not dialog:
            return

        if dialog.action == "snooze":
            self._snooze_minutes = normalize_snooze_minutes(dialog.snooze_minutes)
            self._snooze_until = datetime.datetime.now() + datetime.timedelta(minutes=self._snooze_minutes)
            self._snooze_ringtone_path = normalize_alarm_ringtone(dialog.ringtone_path)
            self._snooze_alarm_id = dialog.alarm_id
        else:
            self._clearSnooze()

        self._alarm_dialog = None
        self._alarm_dialog_open = False
        if self._alarm_was_playing and not getattr(self.slideshow_page, "is_fixed", False):
            self.slideshow_page.play_timer.start()
        self._alarm_was_playing = False
        self.refreshAlarmStatus()

    def _clearSnooze(self):
        self._snooze_until = None
        self._snooze_ringtone_path = ""
        self._snooze_minutes = SNOOZE_MINUTES
        self._snooze_alarm_id = ""

    def _snoozeSourceAlarm(self):
        if not self._snooze_alarm_id:
            return None

        for alarm in self.settings_page.current_alarms:
            if alarm.get("id") == self._snooze_alarm_id:
                return alarm
        return None

    def _snoozeRingtonePath(self):
        alarm = self._snoozeSourceAlarm()
        if alarm:
            return normalize_alarm_ringtone(alarm.get("ringtone_path", ""))
        return normalize_alarm_ringtone(self._snooze_ringtone_path)

    def _snoozeSourceAlarmActive(self):
        ...
        alarm = self._snoozeSourceAlarm()
        return bool(alarm and alarm.get("enabled", True))

    def _cancelInvalidSnooze(self):
        if self._snooze_until and not self._snoozeSourceAlarmActive():
            self._clearSnooze()
    ...
    def _setDefaultNormalGeometry(self):
        screen_geometry = current_screen_geometry(self, available=True)
        width = min(1280, max(960, int(screen_geometry.width() * 0.70)))
        height = int(width * 3 / 4)
        max_height = max(600, screen_geometry.height() - 80)
